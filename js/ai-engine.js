/**
 * AI 知识处理引擎（浏览器版）
 * Enterprise Knowledge AI Engine - Browser Edition
 *
 * 核心能力：
 * 1. 文档内容提取（纯前端，文件不离开浏览器）
 * 2. 智能分类与标签
...
class AIEngine {
  ...
  async extractContent(file) {
    ...
    try {
      if (['.txt', '.md', '.json', '.csv', '.log'].includes(ext) || (file.type && file.type.startsWith('text/'))) {
        return await file.text();
      } else if (ext === '.pdf') {
        const buffer = await file.arrayBuffer();
        return this.extractTextFromPDF(new Uint8Array(buffer), name);
      } else if (['.doc', '.docx', '.pptx', '.ppt', '.xlsx', '.xls'].includes(ext)) {
        // Office 文件本质是 zip，尝试从文本流中抽取可读内容
        const buffer = await file.arrayBuffer();
        const str = new TextDecoder('utf-8', { fatal: false }).decode(buffer);
        const text = this.extractTextFromXMLString(str);
        return text || `[Office文件 ${name}，大小 ${file.size} 字节，浏览器端仅支持部分文本提取，建议上传 txt/md 格式]`;
      } else {
        // 其他类型尝试作为文本读取
        try {
          return await file.text();
        } catch {
          return `[二进制文件: ${name}, 大小: ${file.size}字节]`;
        }
      }
    } catch (error) {
  ...
  extractTextFromPDF(bytes, name) {
    ...
    return text.trim() || `[PDF文件 ${name}，大小 ${bytes.length}字节，浏览器端无法提取文本，建议上传 txt/md 格式]`;
  }

  extractTextFromXMLString(str) {
    let text = '';
    try {
      const regex = /<(?:w:t|a:t|s:t)[^>]*>([^<]+)<\/(?:w:t|a:t|s:t)>/g;
      let match;
      while ((match = regex.exec(str)) !== null) {
